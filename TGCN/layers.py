...
class STBlock(nn.Module):
    def __init__(self,in_channels, n_units_gc, n_units_lstm, sequence_len):
        ...
        self.gcn = GCNConv(in_channels =in_channels, out_channels  =  n_units_gc)
        
        self.rnn = RNNEncoder(input_feature_len = n_units_gc,
...
#         nodes_times = torch.cat([ gap(concat_x, data.batch)], dim=1)
        rnn_out,hid_st  = self.rnn(nodes_times.transpose(1, 2))
